...
def fetch_frost_data(start_date="2023-11-01", end_date="2024-04-30"):
    ...
    try:
        logger.info(
            f"Starting fetch_frost_data med start_date={start_date}, end_date={end_date}"
        )
        ...
        logger.info(f"FROST_CLIENT_ID: {FROST_CLIENT_ID[:8]}...")
        ...
        if r.status_code == 200:
            data = r.json()
            logger.info(f"Data mottatt, antall elementer: {len(data.get('data', []))}")
        ...
        for _, period in sorted_periods.iloc[1:].iterrows():
            ...
            if time_gap <= max_gap:
                ...
                current_period["max_risk_score"] = max(
                    current_period["max_risk_score"], period["max_risk_score"]
                )
