...
# Maksgrenser for visualisering
MAX_DATAPOINTS_FOR_PLOT = 10000
MAX_BARS_FOR_PLOT = 1000
SAMPLE_TARGET = 5000


def _safe_plot_layout(fig: plt.Figure) -> None:
    """Sikker layout-justering med feilhåndtering."""
...
def _downsample_dataframe(df: pd.DataFrame, max_points: int = SAMPLE_TARGET) -> pd.DataFrame:
    """Reduser datasett til maksimalt antall punkter for visualisering."""
...
class LiveConditionsChecker:
    ...
    def get_current_weather_data(self, hours_back: int = 24, start_date: str | None = None, end_date: str | None = None) -> pd.DataFrame | None:
        ...
        try:
            ...
            # Utvidede elementer for bedre væranalyse - ALLE 15 VALIDERTE KJERNELEMENTER
            elements = [
                'air_temperature',
                'wind_speed',
    ...
    def analyze_slippery_road_risk(self, df: pd.DataFrame) -> dict:
        ...
        humidity = latest.get('relative_humidity', None)  # Brukes for fremtidig utvidelse
        ...
        if 'surface_snow_thickness' in last_6h.columns and len(last_6h) >= 2:
            try:
                ...
                if pd.notna(s0) and pd.notna(s1):
                    snow_increase = (s1 - s0) >= 1.0
            except Exception:
                snow_increase = False

        # Regn-på-snø scenario (hovedfokus)
        mild_weather = 0 <= current_temp <= 4
        existing_snow = current_snow >= 5
        rain_now = current_precip >= 0.3

        # Is/rimfrost-kriterier med utvidede data
        ice_risk = False
        frost_risk = False
        if surface_temp is not None and dew_point is not None:
            ice_risk = surface_temp <= 0 and current_temp > -1
            frost_risk = surface_temp <= 0 and abs(current_temp - dew_point) < 2
            
            # Bruk humidity for forbedret rimfrost-deteksjon (ny funksjonalitet)
            if humidity is not None and humidity > 90 and frost_risk:
                frost_risk = True  # Høy fuktighet forsterker rimfrost-risiko

        # Temperaturovergang-scenario
        recent_temp_rise = False
        if len(last_6h) > 3:
            temp_change = last_6h['air_temperature'].diff().max()
            recent_temp_rise = temp_change > 1
        ...
        # Klassifisering med utvidede kriterier og snøfalls-unntak
        if snow_increase:
            return {
                "risk_level": "low",
    ...
    def create_precipitation_classification_plot(self, df: pd.DataFrame) -> plt.Figure:
        ...
        # Sikkerhetsjekk for å unngå for store bilder
        if len(df_classified) > 10000:
            st.warning(f"⚠️ Stort datasett ({len(df_classified)} punkter) - reduserer for visualisering")
            # Sample ned til maksimalt 5000 punkter for visualisering
            sample_rate = len(df_classified) // 5000
            df_classified = df_classified.iloc[::sample_rate].copy()

        # Optimalisert figure-oppsett med sikker størrelse
        try:
            fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(15, 12),
                                               gridspec_kw={'height_ratios': [1.5, 1.5, 2], 'hspace': 0.25})
        except Exception as e:
            st.error(f"❌ Feil ved opprettelse av figur: {e}")
            # Fallback til enklere figure
            fig, ax = plt.subplots(1, 1, figsize=(12, 6))
            ax.text(0.5, 0.5, f'Feil ved visualisering:\n{str(e)[:100]}...', 
                   ha='center', va='center', fontsize=12, color='red')
            ax.set_title('Nedbørtype-klassifisering (Feil)', fontsize=14, fontweight='bold')
            return fig
        ...
        if precip_col:
            ...
            # Optimalisert nedbør-bars med adaptive bredde - fikset for store datasett
            # Begrens antall bars for å unngå bildefeil med for mange piksler
            max_bars = 1000  # Maksimalt 1000 bars for å unngå bildestørrelse-feil
            if len(times) > max_bars:
                # Reduser antall datapunkter ved å sample
                step = len(times) // max_bars
                times_sampled = times[::step]
                precip_sampled = precip_data[::step]
                bar_width = pd.Timedelta(minutes=max(30, min(120, len(times_sampled)//5)))
            else:
                times_sampled = times
                precip_sampled = precip_data
                bar_width = pd.Timedelta(minutes=max(15, min(60, len(times)//10)))
            
            ax1_twin.bar(times_sampled, precip_sampled, alpha=0.7, color='royalblue',
        ...
        if df is not None and len(df) > 0:
            ...
            if len(plot_data) > 0:
                ...
                if VALIDATED_LOGIC_AVAILABLE:
                    try:
                        # Sikkerhetsjekk før visualisering
                        if len(df) > 50000:
                            st.warning(f"⚠️ Stort datasett ({len(df)} punkter) - kan ta tid å visualisere")
                            st.info("💡 Vurder å velge en kortere tidsperiode for bedre ytelse")
                        
                        precipitation_fig = checker.create_precipitation_classification_plot(df)
                        st.pyplot(precipitation_fig)
                        plt.close(precipitation_fig)  # Frigjør minne
                        ...
                        st.error(f"❌ **Feil ved generering av nedbørtype-klassifisering:** {str(e)}")
                        st.info("💡 Dette kan skyldes for stort datasett. Prøv en kortere tidsperiode.")
                        
                        # Vis enkel statistikk i stedet
                        try:
                            df_classified = checker.classify_precipitation_types(df)
                            type_counts = df_classified['precipitation_type'].value_counts()
                            
                            st.write("📊 **Nedbørtype-statistikk (uten visualisering):**")
                            for ptype, count in type_counts.items():
                                percentage = (count / len(df_classified)) * 100
                                st.write(f"- **{ptype.replace('_', ' ').title()}:** {count} observasjoner ({percentage:.1f}%)")
                        except Exception as e2:
                            st.error(f"❌ Kunne ikke generere statistikk: {str(e2)}")
                else:
                    st.error("❌ **Nedbørtype-klassifiseringen krever validert logikk-modul**")
