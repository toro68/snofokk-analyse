...
def classify_maintenance_purpose(row):
    ...
    # 6. UNØDVENDIG BEHANDLING (få, korte episoder uten værgrunn)
    return {
...
def test_ml_against_maintenance_logic():
    ...
    for idx, row in maintenance_df.iterrows():
        ...
        if weather_dependent:
            ...
            if temp != 999:  # Har temperaturdata
                ...
                print("⚠️  Ingen temperaturdata - kan ikke teste ML")
        else:
            print("⏭️  Hopper over - ikke væravhengig")
    ...
    if total_weather_tests > 0:
        ...
        if ml_accuracy >= 75:
            ...
            print("⚠️  AKSEPTABEL ML-ytelse - kan forbedres")
        else:
            print("❌ DÅRLIG ML-ytelse - trenger betydelig forbedring")
    else:
        print("⚠️  Ingen væravhengige episoder testet")

    print("\n💡 VIKTIGE INNSIKTER:")
    print("1. Mange brøytinger er IKKE væravhengige (fredagsrutiner, inspeksjoner)")
    print("2. ML bør kun evalueres mot væravhengige episoder")
    print("3. Kategorisering av vedlikeholdsformål er kritisk for riktig evaluering")
