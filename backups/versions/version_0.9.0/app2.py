...
def show_settings():
    ...
    if not settings_df.empty:
        ...
        for _, settings in settings_df.iterrows():
            with st.expander(
                f"📊 {settings['name']} ({settings['timestamp'].strftime('%Y-%m-%d %H:%M')})"
            ):
                col1, col2 = st.columns(2)
                ...
                with col1:
                    ...
                    if settings.get("critical_periods"):
                        st.metric(
                            "Antall kritiske perioder", settings["critical_periods"]
                        )
                        st.metric(
                            "Total varighet",
                            f"{settings.get('total_duration', 0)} timer",
                        )
                        st.metric(
                            "Gjennomsnittlig risiko",
                            f"{settings.get('avg_risk_score', 0):.1f}",
                        )
        ...
        if df is not None:
            ...
            with st.spinner("Optimaliserer parametre..."):
                ...
                if st.button("Bruk optimaliserte parametre"):
                    ...
                    st.success(
                        "Parametre oppdatert! Analysen vil nå bruke de optimaliserte verdiene."
                    )
                    st.rerun()
