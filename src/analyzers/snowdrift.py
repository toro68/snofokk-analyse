...
class SnowdriftAnalyzer(BaseAnalyzer):
    ...
    def _winter_analysis(self, df: pd.DataFrame) -> AnalysisResult:
        ...
        wind_gust = self._safe_get(latest, 'wind_gust')  # NY: vindkast
        wind_dir = self._safe_get(latest, 'wind_from_direction')
        snow = self._safe_get(latest, 'surface_snow_thickness', 0)
        ...
        # Sjekk løssnø-tilgjengelighet
        loose_snow = self._check_loose_snow(df)
        
        # Sjekk snøendring
        snow_change = self._calculate_snow_change(df)
        
        # Sjekk kritisk vindretning (SE-S: 135-225°)
    ...
    def _calculate_snow_change(self, df: pd.DataFrame) -> float:
        ...
        return snow.iloc[-1] - snow.iloc[-2]
